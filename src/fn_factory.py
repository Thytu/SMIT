...
def __access_value(dict_to_access, key_as_str):
    if "." in key_as_str:
...
def is_shorter_than_model_max_length(columns: List[str], dataset, tokenizer, safe_padding: int = 0, **kwargs):
    ...
    for column in columns:
        dataset = dataset.filter(
            lambda sample: len(__access_value(sample, column)) < model_max_length - safe_padding,
...
def is_longer_than(columns: List[str], value, dataset, **kwargs):
    ...
    for column in columns:
        dataset = dataset.filter(
            lambda sample: len(__access_value(sample, column)) > value,
            num_proc=max(1, os.cpu_count() - 1),
        )
